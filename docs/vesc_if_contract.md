...
| `void (*imu_get_gyro)(float *gyro)` | `imu.c:44` | 1× семпл | **да** | Буфер обновляется до вызова callback | Гироскоп в **°/с** — не рад/с, см. ниже | < 1 мкс (memcpy 12 Б) | **NaN здесь необратимо отравляет фильтр** — см. §6 |
...
> **Единицы гироскопа различаются у двух функций, и это не описка.**
> `imu_get_gyro()` отдаёт **°/с**, а массив `gyro` в callback
> `imu_set_read_callback` — **рад/с**. Ранее в этой таблице для
> `imu_get_gyro()` были указаны рад/с; это было неверно и исправлено на
> v0.6D.1 по исходникам `vedderb/bldc`: `m_gyro` конвертируется через
> `DEG2RAD_f()` перед подачей в AHRS, а в callback уходит уже
> конвертированный `gyro_rad`. Подробный разбор с цитатами —
