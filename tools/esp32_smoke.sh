...
if [ -n "$NM" ] && [ -f "$ELF" ]; then
    ...
    # До v0.7A драйвера TWAI в сборке не было вовсе, и проверка искала любой
    # символ twai_*. С появлением пассивного приёма это стало бы ложным
    # срабатыванием, поэтому проверка уточнена, а не ослаблена: под запретом
    # именно ПЕРЕДАЧА кадров приложением.
    #
    # twai_transmit в образе отсутствует — его выбрасывает сборщик мусора
    # компоновщика, потому что в пассивном профиле функции передачи не
    # объявлены и вызвать их неоткуда. Символы twai_hal_*tx* остаются: их
    # тянет обработчик прерывания, общий для приёма и передачи. Выполниться
