...
## Статус: Этап 0.2 — контракт и host-харнесс готовы

Реальный вывод на моторы не активирован. Готовы: аудит зависимостей, контракт
платформенного слоя, mock-реализация «логического мотора» и host-харнесс, в котором
**неизменённые исходники Refloat собираются и исполняются на десктопе**.
