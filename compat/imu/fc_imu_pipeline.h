...
// ------------------------------------------- каноническая шкала времени IMU
//
// ОДИН источник истины о том, «когда был последний семпл». До v0.9B таких
// источников было три — у трассировки зазоров, у супервизора и у модуля
// здоровья, — и, хотя все три питались одним значением, сравнивать их
// показания было нельзя: каждый обновлялся по своему поводу.
//
// Расхождение, из-за которого это заведено, разобрано в
// docs/imu_time_model.md: супервизор защёлкивал отказ по состоянию здоровья
// (одиночный сбой чтения), а не по возрасту семпла, и порог 40 мс при этом
// не участвовал вовсе. Пока «последнее время» существует в нескольких
// экземплярах, такие вещи выглядят как противоречие приборов.
typedef struct {
    uint64_t last_valid_us;  // отметка последнего ПРИНЯТОГО семпла
    uint64_t prev_valid_us;  // предыдущего принятого
    uint32_t last_gap_us;    // между ними
    uint32_t sequence;       // монотонный номер принятого семпла
    uint32_t last_verdict;   // FcImuPipeVerdict последнего опроса
    uint64_t last_poll_us;   // время последнего опроса, независимо от исхода
} FcImuTimeline;

/**
 * Каноническая шкала. Читать только отсюда: собственная копия «последнего
 * времени» в другом модуле снова создаст два прибора с разными показаниями.
 */
