...
void app_main(void) {
    ...
    if (can_rx) {
        // Повторы, а не одна попытка. Контроллер TWAI после подъёма ещё
        // синхронизируется с шиной, и первые передачи не проходят: при
        // неудаче Refloat молча остаётся с заглушкой малых пределов, и
        // каждый последующий замер идёт с неверным потолком тока.
        // Задержка платится только в случае неудачи.
