...
typedef struct {
    ...
    // Какая именно проверка правдоподобия отвергла семпл. Заведено на v0.9B:
    // во время моторных испытаний отказ приходил от отвергнутых семплов, и
    // без этой разбивки нельзя было сказать, что именно в них не так.
    //
    // Арифметика границ показывает, что достижима только нижняя по модулю
    // ускорения: верхняя равна 7.27 g при физическом пределе датчика 6.93 g,
    // а гироскопная — 510 °/с при насыщении на 500. Счётчики верхних границ
    // обязаны оставаться нулевыми; ненулевой означает, что изменились шкалы.
    uint64_t invalid_accel_low;
