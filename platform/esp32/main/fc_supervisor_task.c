...
static void supervisor_task(void *arg) {
    ...
    for (;;) {
        ...
        // Пока физический датчик не подключён к контуру, его состояние
        // NOT_INITIALIZED не является отказом системы: контур работает от
        // mock-IMU (ТЗ v0.6A §29). Отказом считается именно поломка живого
        // датчика после того, как он однажды заработал.
