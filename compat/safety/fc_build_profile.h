...
// ------------------------------------ замкнутый контур на реальный мотор (v0.9G §13)
//
// Отдельный флаг, а НЕ следствие профиля. «Мотор может крутиться по явной
// команде оператора» и «мотором правит контур балансировки» — разные риски:
// в первом случае величину и длительность задаёт человек, во втором их задаёт
// программа, и ошибка в ней становится непрерывным моментом.
//
// Поэтому одного MOTOR_EXPERIMENTAL недостаточно. Требуются одновременно:
//   * профиль с backend-ом выхода на мотор;
//   * подтверждение вывешенного колеса, написанное вслух;
//   * профиль шины с транспортом команд мотору;
//   * и сам этот флаг.
//
// Ни одно из условий не выводится из других: каждое приходится задать.
#if defined(FLOATCORE_REFLOAT_REAL_MOTOR_LOOP)
#if !FC_MOTOR_BACKEND_AVAILABLE
#error "Замкнутый контур требует профиля с backend-ом выхода на мотор"
#endif
#if !defined(FLOATCORE_MOTOR_EXPERIMENTAL_WHEEL_OFF_GROUND)
#error "Замкнутый контур собирается только с подтверждением вывешенного колеса"
#endif
#if !defined(FLOATCORE_CAN_ACTIVE_MOTOR)
#error "Замкнутый контур требует FLOATCORE_CAN_ACTIVE_MOTOR"
#endif
#define FC_CLOSED_LOOP_AVAILABLE 1
#else
#define FC_CLOSED_LOOP_AVAILABLE 0
#endif
