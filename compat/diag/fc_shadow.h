...
// Допуск при сравнении с пределом тока. Зажатие происходит внутри Refloat над
// float-величиной, и результат оказывается на единицы ULP ниже предела:
// точное `>=` не срабатывало никогда, и разбор показывал 0 % упора там, где
// команда стояла в потолке ESC. Допуск меньше любого различимого тока и
// заведомо больше ошибки округления на величинах порядка единиц ампер.
#define FC_SHADOW_LIMIT_EPS_A 0.001f
...
typedef struct {
    ...
    float pitch;        // градусы
    float balance_pitch; // то, от чего РЕАЛЬНО считает ошибку регулятор
    float roll;
    float pitch_rate;   // °/с
    float setpoint;
    float pid_p;
