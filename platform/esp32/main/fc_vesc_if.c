...
// Точность пробуждения (ТЗ v0.7D §11).
//
// Инструментальный вопрос, который надо было задать раньше: сколько времени
// поток Refloat ПРОСИТ поспать и сколько спит на самом деле. Разница — это
// не его работа и не вытеснение, а задержка механизма пробуждения, и в
// статистику исполнения она попадать не должна.
//
// Считается здесь, а не в fc_timing: fc_timing меряет период и wall-время
// итерации, а это — качество самого сна.
...
static void if_sleep_us(uint32_t us) {
    ...
    if (t) {
        // Периодическое пробуждение по абсолютной отметке (ТЗ v0.7D §12).
        //
        // Refloat просит не «поспать столько-то», а «проснуться в начале
        // следующего периода»: он сам вычитает из запроса время своей
        // итерации (main.c:1058-1059). Это ровно та задача, для которой
        // существует xTaskDelayUntil, и она решает её лучше, чем мы:
        // отметка абсолютная, поэтому ошибка не накапливается, а пробуждение
        // происходит в обработчике системного тика, на том же ядре — без
        // посредника.
        //
        // Чем это лучше прежнего esp_timer. Его обработчик исполняется в
        // ЗАДАЧЕ esp_timer, закреплённой за ядром 0, а потоки Refloat живут
        // на ядре 1. Каждое пробуждение было межъядерным уведомлением.
        // Измерено: задержка 236 мкс в среднем и до 964 в худшем случае, с
        // отдельным горбом гистограммы на 700-900 мкс, примерно половина
        // которого исчезала при остановке приёма CAN. Поскольку Refloat
        ...
        if (slept == pdFALSE) {
            ...
            // нечем. Учитываем как нулевой сон, чтобы не выдумывать цифру.
            fc_sched_note_sleep((size_t) (t - S.threads), actual, actual);
        } else {
            fc_sched_note_sleep((size_t) (t - S.threads), us, actual);
        }
    } else {
    ...
    if (t) {
        ...
        //
        // Период отмечается ЗДЕСЬ, а не перед сном (ТЗ v0.7D §11). Отметка
        // перед сном мерила бы «пробуждение плюс исполнение», и разброс
        // времени исполнения выглядел бы как опоздание планировщика: при
        // идеально периодическом пробуждении половина периодов оказывалась
        // бы «поздними» просто потому, что предыдущая итерация была короче.
        // Период потока — это интервал между пробуждениями, и мерить его
        // надо в момент пробуждения.
        FcTimingChannel ch = (t == &S.threads[0]) ? FC_TIMING_MAIN : FC_TIMING_AUX;
        fc_timing_tick(ch);
...
void fc_vesc_if_init(void) {
    ...
    // из ранних этапов, для этой батареи дававшая Refloat порог отката по
    // низкому напряжению 3.0 x 15 = 45 В, то есть ВЫШЕ напряжения полностью
    // заряженной батареи (42 В). Откат был бы включён всегда.
    S.cfg_int[CFG_PARAM_si_battery_cells] = fc_battery_cell_count();
